"""Convert :class:`chat.CardElement` values to Linear-flavored markdown.

Python port of upstream ``packages/adapter-linear/src/cards.ts``. Linear has
no native rich-card surface, so cards are flattened to clean markdown: bold
title and subtitle, fields as ``**Label:** Value`` pairs, buttons as markdown
links (link buttons) or bold-bracketed text (action buttons).
"""
...
def card_to_linear_markdown(card: dict[str, Any]) -> str:
    """Render a :class:`chat.CardElement` dict to Linear-flavored markdown."""
...
__all__ = ["card_to_linear_markdown", "card_to_plain_text"]
